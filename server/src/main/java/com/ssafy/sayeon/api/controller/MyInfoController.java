...
public class MyInfoController {
	...
	public ResponseEntity<? extends BaseResponseBody> readUserInfo(@PathVariable("userId") String userId){
		return ResponseEntity.status(200).body(AdvancedResponseBody.of(200,	"유저 정보 조회 성공", userService.getUserByUserId(userId)));
	}
	...
	public ResponseEntity<? extends BaseResponseBody> modifyNickname(@PathVariable("userId") String userId, @RequestHeader("Authorization") String token, @RequestBody UserProfileUpdateReq updateInfo){ //수정 필요
		...
		if (!userService.getUserByUserId(userId).getUserProfile().getNickname().equals(updateInfo.getNickname())  // 기존 닉네임과 다른 경우(변동사항이 있는 경우)
			 && userService.getUserProfileByNickname(updateInfo.getNickname()) != null) { // 닉네임 중복 검사
